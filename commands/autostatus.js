    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (err.message?.includes('rate-overlimit') && i < retries - 1) {
                console.log(`⚠️ Rate limit hit, retrying in ${delay}ms... - autostatus.js:46`);
...
async function autoStatusCommand(sock, chatId, msg, args) {
    try {
        ...
        console.error('Error in autostatus command: - autostatus.js:103', error);
        await sock.sendMessage(chatId, { 
...
async function viewStatus(sock, key) {
    if (key?.remoteJid === 'status@broadcast') {
        ...
        console.log(`✅ Viewed status from: ${sender.split('@')[0]} - autostatus.js:121`);
    }
}
...
async function handleStatusUpdate(sock, status) {
    try {
        ...
        console.error('❌ Error in auto status view: - autostatus.js:142', error.message);
    }
}
